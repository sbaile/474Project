...
for (category in names(symbols_list)) {
  ...
  for (symbol in symbols) {
    # Use the original symbol name to get the data
    stock_returns[[paste(category, symbol, sep = "_")]] <- dailyReturn(Cl(get(symbol)))
  }
}
...
# Set column names back to the original stock symbols for the merged data
colnames(portfolio_returns_data) <- unlist(symbols_list)

# Display the portfolio returns data
...
for (category in names(symbols_list)) {
  ...
  # Loop through each symbol in the category
  for (symbol in symbols) {
    # Create the hierarchical name for the current symbol
    hierarchical_symbol <- paste(category, symbol, sep = "_")
    
    # Adjust returns using buy signals
    if (hierarchical_symbol %in% colnames(buy_signals)) {
      adjusted_returns[, hierarchical_symbol] <- stock_returns[[hierarchical_symbol]] * buy_signals[[hierarchical_symbol]]
    } else {
      warning(paste("Buy signals for", hierarchical_symbol, "not found."))
    }
  }
}
